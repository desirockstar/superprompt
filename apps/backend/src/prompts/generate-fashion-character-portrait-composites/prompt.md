---
title: "generate fashion character portrait composites"
slug: "generate-fashion-character-portrait-composites"
category: "uncategorized"
status: "approved"
isViral: false
isNano: false
version: 1
---

---
title: "🧸 Generate Fashion Character Portrait Composites"
source: godofprompt.ai
slug: "promptsgenerate-fashion-character-portrait-composites"
---

{
  "image_generation": {
    "quality": "hyper-realistic, high-detail, professional fashion photoshoot",
    "face": {
      "preserve_original": true,
      "reference_match": true,
      "description": "Use the exact same face from the uploaded photo without altering any facial features or identity."
    },
    "subject": {
      "description": "A stylish young woman with the same face as the reference photo.",
      "clothing": [OUTFIT],
      "pose": "standing with one arm resting casually on a giant 3D-rendered [CHARACTER]",
      "expression": "natural, confident, casual fashion look"
    },
    "character_element": {
      "name": "[CHARACTER]",
      "type": "giant 3D photorealistic render",
      "style": "clean, glossy, cartoon-accurate but ultra-detailed",
      "interaction": "the woman's arm rests on the character as if posing together"
    },
    "environment": {
      "background": "seamless solid vibrant [COLOR THEME] backdrop",
      "setting": "professional photo studio",
      "lighting": "soft evenly diffused studio lighting with clean highlights and minimal shadows"
    },
    "aesthetic": {
      "style": "modern fashion photography",
      "mood": "playful, bright, stylish",
      "details": [
        "hyperrealistic skin texture",
        "sharp fabric details",
        "accurate lighting reflections on 3D character",
        "smooth studio gradients"
      ]
    }
  }
}

## How to use the prompt

Generates a hyper-realistic AI image of a woman posing with a giant 3D cartoon character in a studio setting. Preserves the exact facial features from an uploaded reference photo while placing the subject in a fashion photoshoot scene. Creates professional studio lighting with vibrant colored backdrops and detailed textures on both the model and 3D character.

## Categories

Photography, Commercial Photography

## Recommended tools

- Nano Banana
